...
void handler(void) {
    ...
    else if (mcause_value == 0x80000016) { // IRQ = 22
        led_display_val = 0;
        // int timeout = 0; // Set a timeout value to prevent infinite loop
        if ((*PS2_ptr & 0x8000)) { // While RVALID is set and timeout not reached
            ...
            if (byte1 == 0xE0 && byte2 == 0xF0) {
                ...
                switch(byte3) {
                    case 0x75: led_display_val = 1; break; // Up
                    case 0x6B: led_display_val = 2; break; // Left
                    case 0x72: led_display_val = 4; break; // Down
                    case 0x74: led_display_val = 8; break; // Right
                }
            }
